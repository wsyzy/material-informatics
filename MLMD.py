...
''')
    select_option = option_menu("MLMD", ["Home Page", "Basic Data", "Feature Engineering","Cluster & ReduceDim", "Regression", "Classification", "Transfer Learning", "Model Inference","Surrogate Optimization","Active Learning","Interpretable Machine Learning","3D metamaterial structure generation"],
...
elif select_option == "3D metamaterial structure generation":
    colored_header(label="3D metamaterial structure generation", description=" ", color_name="violet-90")


    st.subheader('项目介绍')
    st.text('直接在3D体素图像上修改得到新的图像，然后利用有限元计算相应的材料性能，再利用机器学习训练图像与性能的关系，之后便直接利用机器学习模型生成符合性能输入的3D图像，投入3D打印使用')
    image = Image.open('./data/fig7.png')
    st.image(image, caption='项目流程')

    st.subheader('实例结构：拉涨材料')
    st.text('拉胀材料即具有负泊松比的材料，它具有受拉膨胀和(或)受压收缩的力学特性。')
    image = Image.open('./data/fig8.png')
    st.image(image, caption='拉涨材料结构实例')

    st.subheader('Stable Diffusion模型')
    # 纯文本
    st.text('Stable Diffusion是计算机视觉领域的一个生成式大模型，能够进行文生图（txt2img）和图生图（img2img）等图像生成任务。预期采用SD模型来进行3D结构图像生成任务')
    image = Image.open('./data/fig9.png')
    st.image(image, caption='文生图')
    image = Image.open('./data/fig10.png')
    st.image(image, caption='图生图')
    st.subheader('敬请期待...')
